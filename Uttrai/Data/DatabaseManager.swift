...
final class DatabaseManager {
    ...
    convenience init() throws {
        let appSupportURL = FileManager.default.urls(
            ...
            in: .userDomainMask
        ).first!.appendingPathComponent("Uttrai")
        ...
        let dbPath = appSupportURL.appendingPathComponent("uttrai.db").path
    ...
    func observeTranscriptions(
        ...
        return observation.start(in: dbQueue) { _ in
            do {
                let records = try self.fetchRecent()
                onChange(records)
            } catch {
