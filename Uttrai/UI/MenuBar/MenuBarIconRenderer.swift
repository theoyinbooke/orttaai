...
final class MenuBarIconRenderer {
    ...
    static func renderIcon(for state: IconState, size: NSSize = NSSize(width: 18, height: 18)) -> NSImage {
        ...
        case .recording:
            return tintedIcon(symbolName: "waveform.circle.fill", color: NSColor.Uttrai.accent, size: size)
        case .processing:
            return tintedIcon(symbolName: "waveform.circle.fill", color: NSColor.Uttrai.accent, size: size)
    ...
    private static func tintedIcon(symbolName: String, color: NSColor, size: NSSize) -> NSImage {
        let symbol = NSImage(systemSymbolName: symbolName, accessibilityDescription: "Uttrai")!
    ...
    private static func downloadingIcon(progress: Double, size: NSSize) -> NSImage {
        let image = NSImage(size: size, flipped: false) { rect in
            // Base icon
            let baseIcon = NSImage(systemSymbolName: "waveform.circle", accessibilityDescription: "Uttrai")!
            ...
            path.lineWidth = 1.5
            NSColor.Uttrai.accent.setStroke()
            path.stroke()
